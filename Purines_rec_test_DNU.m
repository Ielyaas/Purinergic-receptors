...
solver = @(x,t)Purine_rec(x,t,param);
opts = odeset('RelTol',1e-9,'AbsTol',1e-12);
[~,Y] = ode15s(solver,[0 250],[1 1 0.0805296 3.861011 0.70395 0],opts);
...
hold on
yyaxis right
plot(T,p,'color',[0 0.6 0],'LineWidth',3)
xlabel('time (s)')
ylabel('[IP_3] \muM')
hold on
...
function out = Purine_rec(~,in,param)
...
out(3) = (Jipr - Jserca + param.delta.*(Jin - Jpm))./param.tau_c;
