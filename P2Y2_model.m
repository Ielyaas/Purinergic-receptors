...
param.Vplc = 0.015; param.Kplc = 0.11; param.Vpkc = 0.1;
param.Kpkc = 0.3;
...
[~,Y] = ode15s(solver,[0 4800],[0 0 0.0805296 3.861011 0.70395 0 0],opts);
...
kr = ((R1*param.A1/(param.K1 + param.A1)) + ...
     ...
     (param.Vplc + param.lambda_4.*Q_i).*(param.lambda_5*alpha0_til...
...
saveas(gcf,'Fig5sb','epsc')

% figure(2)
% plot(T,Q_i,'Color',[0 0.447 0.741],'LineWidth',3)
% xlabel('time (s)')
% ylabel('[Ca^{2+}_i] (\muM)')
% % title('Closed cell')
% ax=gca;
...
% hold off
% set(gcf,'position',[10,10,2000,1400]) %[xpos, ypos, Width, Height]
% % saveas(gcf,'Fig4sb','epsc')

% tilefigs
%% FUNCTION FILE

function out = Pur_rec(t,in,param)
...
out(7) = param.Tscale.*(kr.*(param.L_t - L) - param.k_r.*L)./param.tau_L;
out = out';
