...
param.Vplc = 0.2; param.Kplc = 0.2; param.Vpkc = 0.1;
param.Kpkc = 0.3;
...
[T,Y] = ode15s(solver,[0 500],[1 1 0.0805296 3.861011 0.70395 0],opts);
...
out(1) = param.T_rescale.*((1-R1)*rho1_d - R1.*Q_tilde_1*rho1_p);
out(2) = param.T_rescale.*((1-R2)*rho2_d - R2.*Q_tilde_2*rho2_p);
out(3) = param.T_rescale.*(Jipr - Jserca + Jin - Jpm)./param.tau_c;
out(4) = param.T_rescale.*(Jin - Jpm);
out(5) = param.T_rescale.*(h_inf - h)./tau;
out(6) = param.T_rescale.*(param.kL.*L - param.k_i.*p);
out = out';
