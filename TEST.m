...
param.Vplc = 0.08; param.Kplc = 0.11; param.Vpkc = 0.15;
param.Kpkc = 0.3;
...
[~,Y] = ode15s(solver,[0 5020],[1 0.0805296 3.861011 0.70395 0],opts);
...
hold off
set(gcf,'position',[10,10,1250,800]) %[xpos, ypos, Width, Height]
% saveas(gcf,'Pur_slide2b','epsc')

% tilefigs

%% FUNCTION FILE

function out = P2Y1_test(~,in,param)
...
out(1) = (1-R1)*rho_d - R1.*Q_1*rho_p;
out(2) = (Jipr - Jserca + Jin - Jpm)./param.tau_c;
out(3) = Jin - Jpm;
out(4) = (h_inf - h)./tau;
% out(5) = (L - nu_deg.*p)./tau_p;
out(5) = (L - param.k_i.*p);
out = out';         
