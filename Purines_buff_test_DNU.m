...
param.Kpkc = 0.3;
param.bt = 50; param.Kbuff = 2;
param.kminb = 1000; param.kplusb = 500;
param.bte = 5000; param.F = 1500;
...
solver = @(x,t)Purine_buff(x,t,param);
opts = odeset('RelTol',1e-9,'AbsTol',1e-12);
[T,Y] = ode15s(solver,[0 1000],[1 1 0.0805296 3.861011 0.70395 0],opts);
...
function out = Purine_buff(t,in,param)
...
% Buffer
b = param.bt.*c./(param.Kbuff + c);
Fbuff = 1 + param.bte*param.F./(param.F + ce).^2;
...
out(3) = (Jipr - Jserca + param.delta.*(Jin - Jpm) +...
         param.kminb*b - param.kplusb.*c.*(param.bt - b))./param.tau_c;
out(4) = param.delta.*(Jin - Jpm)./Fbuff;
out(5) = (h_inf - h)./tau;
