...
param.Vplc = 0.003; param.Kplc = 0.11; param.Vpkc = 0;
param.Kpkc = 0.3;
...
[~,Y] = ode15s(solver,[0 1940],[1 1 0.0805296 3.861011 0.70395 0],opts);
[T,Y] = ode113(solver,[0 250],Y(end,:),opts);
% Ca_plot1{1,1} = Y(:,3);

% param.Ktau = 0.2;
% solver = @(x,t)Pur_rec(x,t,param);
% opts = odeset('RelTol',1e-9,'AbsTol',1e-12);
% [~,Y] = ode15s(solver,[0:0.01:1940],[1 1 0.0805296 3.861011 0.70395 0],opts);
% [T,Y] = ode113(solver,[0:0.01:150],Y(end,:),opts);
% Ca_plot1{1,2} = Y(:,3);
...
% figure(7)
% plot(T,Ca_plot1{1,1},'b','LineWidth',4)
% hold on
% plot(T,Ca_plot1{1,2},'r','LineWidth',4)
% xlabel('time (s)')
% ylabel('[Ca^{2+}_i] \muM')
